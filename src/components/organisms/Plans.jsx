...
const Card = ({ plan }) => (
  <article
    className="relative shrink-0 snap-start w-[86vw] sm:w-[30rem] lg:w-[34rem]
               overflow-hidden rounded-[28px] border border-black/[0.06]
               bg-[#F4F4F7]"
  >
    <div className="flex items-center gap-6 sm:gap-8">
      {/* Artwork. The invite-only card bleeds top-to-bottom with only a left
          gap; every other card sits its art inside a padded glass tray. */}
      {plan.bleed ? (
        ...
        <div className="grid h-[15rem] sm:h-[17rem] w-[42%] shrink-0 place-items-center p-5 sm:p-7">
          ...
          <img
            src={plan.art}
            alt=""
            className="col-start-1 row-start-1 h-[54%] w-[54%] object-contain"
            loading="lazy"
          />
        </div>
      )}
      ...
      {/* Copy */}
      <div className="flex-1 pr-6 sm:pr-8 py-6">
...
const Plans = () => {
  ...
  // Keep the arrows in step when the user swipes or drags instead of clicking.
  ...
  return (
    <section id="plans" className="w-full bg-white">
      <div className="mx-auto max-w-6xl px-6 sm:px-10 lg:px-16 py-24 md:py-32">
        ...
        {/* Track. Native scroll-snap so touch swiping feels right, with the
            arrows driving the same scroll on desktop. */}
        <motion.div {...reveal(0.15)}>
          <div
            ref={trackRef}
            onScroll={onScroll}
            className="flex gap-5 sm:gap-6 overflow-x-auto snap-x snap-mandatory scroll-smooth
